...
bool CRedisConnection::ConnectToRedis(const std::string &strHost, int nPort, int nTimeout, const std::string& strPassword)
{
    if (m_pContext)
        redisFree(m_pContext);
...
bool CRedisConnection::Reconnect()
{
    if (!m_pRedisServ->m_strHost.empty() &&
        ConnectToRedis(m_pRedisServ->m_strHost, m_pRedisServ->m_nPort, m_pRedisServ->m_nCliTimeout, m_pRedisServ->m_strPassword))
        return true;

    for (auto &hostPair : m_pRedisServ->m_vecHosts)
    {
        if (ConnectToRedis(hostPair.first, hostPair.second, m_pRedisServ->m_nCliTimeout, m_pRedisServ->m_strPassword))
        {
...
CRedisServer::CRedisServer(const std::string &strHost, int nPort, int nTimeout, int nConnNum, const std::string& password)
    : m_strHost(strHost), m_nPort(nPort), m_nCliTimeout(nTimeout), m_nSerTimeout(60), m_nConnNum(nConnNum)
...
bool CRedisClient::Initialize(const std::string &strHost, int nPort, int nTimeout, int nConnNum, const std::string &pass)
{
    ...
    CRedisServer *pRedisServ = new CRedisServer(m_strHost, m_nPort, m_nTimeout, m_nConnNum, m_strPassword);
    ...
    for (size_t i = 0; i < m_vecRedisServ.size(); ++i)
    {
            ...
            redisCmd.FetchResult(BIND_SLOT(&vecSlot)) == RC_SUCCESS)
        {
            for (auto &slotReg : vecSlot)
            {
                ...
                {
                    pSlotServ = new CRedisServer(slotReg.strHost, slotReg.nPort, m_nTimeout, m_nConnNum, m_strPassword);
