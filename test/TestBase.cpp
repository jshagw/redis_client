...
bool CTestBase::StartTest(const std::string &strHost, const std::string &password)
{
    bool bSuccess = false;
    std::cout << "start to test base command" << std::endl;
    if (!m_redis.Initialize(strHost, 6379, 2, 10, password))
...
bool CTestBase::Test_Set()
{
    if (!InitStringEnv(2, 1) || !InitListEnv(1, 1))
        return PrintResult("set InitStringEnv", false);
